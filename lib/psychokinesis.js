...
module.exports.createServer = function (options, requestListener) {
    ...
    let dns = new DhtStore({
        nodes: (this.options.entryNode && this.options.entryNode.dhtPort) ? [{host: this.options.entryNode.host, port: this.options.entryNode.dhtPort}] : [],
    ...
    if (this.options.enableDns) {
        ...
        dnsServer.listen(53, () => {
            console.log(new Date(), 'dns-server is up');
        });
